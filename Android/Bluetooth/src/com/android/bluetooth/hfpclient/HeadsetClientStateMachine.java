...
final class HeadsetClientStateMachine extends StateMachine {
    private static final String TAG = "LZLB_HeadsetClientStateMachine";
    ...
    private void addCall(int state, String number) {
        Log.e(TAG, "addToCalls state:" + state + " number:" + number);
    ...
    private void removeCalls(int... states) {
        Log.e(TAG, "removeFromCalls states:" + Arrays.toString(states));
    ...
    private void changeCallsState(int old_state, int new_state) {
        Log.e(TAG, "changeStateFromCalls old:" + old_state + " new: " + new_state);
    ...
    private BluetoothHeadsetClientCall getCall(int... states) {
        Log.e(TAG, "getFromCallsWithStates states:" + Arrays.toString(states));
        for (BluetoothHeadsetClientCall c : mCalls.values()) {
            for (int s : states) {
                if (c.getState() == s) {
                    return c;
    ...
    private void updateCallsMultiParty() {
    	Log.e(TAG,"updateCallsMultiParty");
    ...
    private void sendCallChangedIntent(BluetoothHeadsetClientCall c) {
        Log.e(TAG, "sendCallChangedIntent " + c);
    ...
    private void updateCallIndicator(int call) {
        Log.e(TAG, "updateCallIndicator " + call);
    ...
    private void updateCallSetupIndicator(int callsetup) {
        Log.e(TAG, "updateCallSetupIndicator " + callsetup + " " + mPendingAction.first);
    ...
    private void updateCallHeldIndicator(int callheld) {
        Log.e(TAG, "updateCallHeld " + callheld);
    ...
    private void updateRespAndHold(int resp_and_hold) {
        Log.e(TAG, "updatRespAndHold " + resp_and_hold);
    ...
    private void enterPrivateMode(int idx) {
        Log.e(TAG, "enterPrivateMode: " + idx);
    ...
    private HeadsetClientStateMachine(HeadsetClientService context) {
        super(TAG);
		Log.e(TAG, "HeadsetClientStateMachine ");
        mService = context;
    ...
    private class Disconnected extends State {
    	private static final String TAG ="LZL_Disconnected";
        @Override
        public void enter() {
    ...
    private class Connecting extends State {
    	private static final String TAG="LZLB_Connecting";
        @Override
        public void enter() {
        ...
        @Override
        public synchronized boolean processMessage(Message message) {
            Log.e(TAG, "Connecting process message: " + message.what);
        ...
        private void processConnectionEvent(int state, int peer_feat, int chld_feat, BluetoothDevice device) {
        	Log.e(TAG,"processConnectionEvent");
            switch (state) {
                case HeadsetClientHalConstants.CONNECTION_STATE_DISCONNECTED:
    ...
    private class Connected extends State {
        ...
        private void sendActionResultIntent(StackEvent event) {
        	Log.e(TAG,"sendActionResultIntent");
    ...
    private class AudioOn extends State {
    	private static final String TAG="LZLB_HCSM_AudioOn";
        @Override
        public void enter() {
            Log.e(TAG, "Enter AudioOn: " + getCurrentMessage().what);
        ...
        @Override
        public synchronized boolean processMessage(Message message) {
            Log.e(TAG, "AudioOn process message: " + message.what);
            if (DBG) {
                if (mCurrentDevice == null) {
                    Log.d(TAG, "ERROR: mCurrentDevice is null in Connected");
                    return NOT_HANDLED;
    ...
    private void onLastVoiceTagNumber(String number) {
    	Log.e(TAG,"onLastVoiceTagNumber");
