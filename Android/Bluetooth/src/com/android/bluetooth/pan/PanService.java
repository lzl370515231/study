...
public class PanService extends ProfileService {
    private static final String TAG = "LZLB_PanService";
    ...
    private static class BluetoothPanBinder extends IBluetoothPan.Stub
		...
		private static final String TAG ="LZLB_BluetoothPanBinder";
        ...
        public void setBluetoothTethering(boolean value) {
        	Log.e(TAG,"setBluetoothTethering");
            PanService service = getService();
    ...
    boolean isPanNapOn() {
        if(DBG) Log.e(TAG, "isTetheringOn call getPanLocalRoleNative");
    ...
    void setBluetoothTethering(boolean value) {
        if(DBG) Log.e(TAG, "setBluetoothTethering: " + value +", mTetherOn: " + mTetherOn);
    ...
    private void onConnectStateChanged(byte[] address, int state, int error, int local_role,
            int remote_role) {
        if (DBG) {
            Log.e(TAG,"onConnectStateChanged: " + state + ", local role:" + local_role +
    ...
    private void onControlStateChanged(int local_role, int state, int error, String ifname) {
        if (DBG)
            Log.e(TAG,"onControlStateChanged: " + state + ", error: " + error + ", ifname: " + ifname);
        if(error == 0)
            mPanIfName = ifname;
    }
    ...
    void handlePanDeviceStateChange(BluetoothDevice device,
        ...
        if(DBG) {
            Log.e(TAG, "handlePanDeviceStateChange: device: " + device + ", iface: " + iface +
        ...
        if (prevState == BluetoothProfile.STATE_DISCONNECTED && state == BluetoothProfile.STATE_DISCONNECTING) {
            Log.e(TAG, "Ignoring state change from " + prevState + " to " + state);
            return;
        }

        Log.e(TAG, "handlePanDeviceStateChange preState: " + prevState + " state: " + state);
        ...
        if (remote_role == BluetoothPan.LOCAL_PANU_ROLE) {
            ...
            Log.e(TAG, "handlePanDeviceStateChange LOCAL_PANU_ROLE:REMOTE_NAP_ROLE state = " +
        ...
        Log.e(TAG, "Pan Device state : device: " + device + " State:" +
                       prevState + "->" + state);
    ...
    private String enableTethering(String iface) {
        if (DBG) Log.e(TAG, "updateTetherState:" + iface);
    ...
    private String createNewTetheringAddressLocked() {
    	Log.e(TAG,"createNewTetheringAddressLocked");
    ...
    private List<BluetoothDevice> getConnectedPanDevices() {
    	Log.e(TAG,"getConnectedPanDevices");
    ...
    private int getPanDeviceConnectionState(BluetoothDevice device) {
    	Log.e(TAG,"getPanDeviceConnectionState");
        BluetoothPanDevice panDevice = mPanDevices.get(device);
        if (panDevice == null) {
            return BluetoothProfile.STATE_DISCONNECTED;
        }
