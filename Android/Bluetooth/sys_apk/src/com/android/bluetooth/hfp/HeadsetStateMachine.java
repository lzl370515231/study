...
final class HeadsetStateMachine extends StateMachine {
    private static final String TAG = "LZLB_HeadsetStateMachine";
    ...
    private class Disconnected extends State {
    	private static final String TAG ="LZLB_Disconnected";
        @Override
        public void enter() {
            Log.e(TAG,"Enter Disconnected: " + getCurrentMessage().what +
        ...
        private void processConnectionEvent(int state, BluetoothDevice device) {
            Log.e(TAG, "processConnectionEvent state = " + state +
                             ", device = " + device);
            switch (state) {
    ...
    private class Pending extends State {
    	private static final String TAG ="LZLB_Pending";
        @Override
        public void enter() {
            Log.e(TAG,"Enter Pending: " + getCurrentMessage().what);
        }

        @Override
        public boolean processMessage(Message message) {
            Log.e(TAG,"Pending process message: " + message.what + ", size: "
        ...
        private void processMultiHFConnected(BluetoothDevice device) {
            Log.e(TAG,"Pending state: processMultiHFConnected");
    ...
    private class Connected extends State {
    	private static final String TAG="LZLB_Connected";
        @Override
        public void enter() {
            ...
            removeDeferredMessages(CONNECT);
           	...
           	Log.e(TAG,"Enter Connected: " + getCurrentMessage().what +
                           ", size: " + mConnectedDevicesList.size());
        ...
        @Override
        public boolean processMessage(Message message) {
            Log.e(TAG,"Connected process message: " + message.what +
                          ", size: " + mConnectedDevicesList.size());
            if (DBG) {
                if (mConnectedDevicesList.size() == 0) {
                    Log.e(TAG,"ERROR: mConnectedDevicesList is empty in Connected");
                    return NOT_HANDLED;
                }
            }
            ...
            switch(message.what) {
                ...
                case STACK_EVENT:
                    StackEvent event = (StackEvent) message.obj;
                    if (DBG) {
                        Log.e(TAG,"event type: " + event.type + "event device : "
                                                  + event.device);
                    }
                    switch (event.type) {
                        case EVENT_TYPE_CONNECTION_STATE_CHANGED:
        ...
        // in Connected state
        private void processConnectionEvent(int state, BluetoothDevice device) {
        Log.e(TAG, "processConnectionEvent state = " + state + ", device = "
                                                           + device);
            switch (state) {
                case HeadsetHalConstants.CONNECTION_STATE_DISCONNECTED:
                    if (mConnectedDevicesList.contains(device)) {
        ...
        private void processAudioEvent(int state, BluetoothDevice device) {
        	Log.e(TAG,"processAudioEvent state:"+state);
            if (!mConnectedDevicesList.contains(device)) {
        ...
        private void processMultiHFConnected(BluetoothDevice device) {
            Log.e(TAG,"processMultiHFConnected");
    ...
    private class AudioOn extends State {
		private static final String TAG ="LZLB_AudioOn";
        @Override
        public void enter() {
            Log.e(TAG,"Enter AudioOn: " + getCurrentMessage().what + ", size: " +
                                  mConnectedDevicesList.size());
        }

        @Override
        public boolean processMessage(Message message) {
            Log.e(TAG,"AudioOn process message: " + message.what + ", size: " +
                                  mConnectedDevicesList.size());
            if (DBG) {
                if (mConnectedDevicesList.size() == 0) {
                    Log.e(TAG,"ERROR: mConnectedDevicesList is empty in AudioOn");
                    return NOT_HANDLED;
                }
            }
        ...
        private void processIntentScoVolume(Intent intent, BluetoothDevice device) {
        	Log.e(TAG, "processIntentScoVolume ");
    ...
    private class MultiHFPending extends State {
    	private static final String TAG ="LZLB_MultiHFPending";
        @Override
        public void enter() {
            Log.e(TAG,"Enter MultiHFPending: " + getCurrentMessage().what +
                         ", size: " + mConnectedDevicesList.size());
        }

        @Override
        public boolean processMessage(Message message) {
            Log.e(TAG,"MultiHFPending process message: " + message.what +
                         ", size: " + mConnectedDevicesList.size());
    ...
    {
    	Log.e(TAG,"processLocalVrEvent  state:"+state);
    ...
    private synchronized void expectVoiceRecognition(BluetoothDevice device) {
    	Log.e(TAG,"expectVoiceRecognition ");
    ...
    {	
    	Log.e(TAG,"getDeviceForMessage what:"+what);
    ...
    private void broadcastAudioState(BluetoothDevice device, int newState, int prevState) {
        ...
        mService.sendBroadcastAsUser(intent, UserHandle.ALL,
                HeadsetService.BLUETOOTH_PERM);
        Log.e(TAG,"Audio state " + device + ": " + prevState + "->" + newState);
    }
    ...
    private void broadcastVendorSpecificEventIntent(String command,
                                                    ...
                                                    BluetoothDevice device) {
                                                    	Log.e(TAG,"broadcastVendorSpecificEventIntent ");
    ...
    {
    	Log.e(TAG,"configAudioParameters ");
    ...
    synchronized boolean initiateScoUsingVirtualVoiceCall() {
    	Log.e(TAG,"initiateScoUsingVirtualVoiceCall ");
    ...
    private void processNoiceReductionEvent(int enable, BluetoothDevice device) {
    	Log.e(TAG,"processNoiceReductionEvent ");
    ...
    private void processSubscriberNumberRequest(BluetoothDevice device) {
    	Log.e(TAG,"processSubscriberNumberRequest ");
        if(device == null) {
    ...
    private boolean processVendorSpecificAt(String atString) {
    	Log.e(TAG,"processVendorSpecificAt ");
    ...
    private void onNoiceReductionEnable(boolean enable,  byte[] address) {
    	Log.e(TAG,"onNoiceReductionEnable ");
    ...
    private void processIntentBatteryChanged(Intent intent) {
    	Log.e(TAG,"processIntentBatteryChanged ");
    ...
    private void processDeviceStateChanged(HeadsetDeviceState deviceState) {
    	Log.e(TAG,"processDeviceStateChanged ");
    ...
    private void processSendClccResponse(HeadsetClccResponse clcc) {
    	Log.e(TAG,"processSendClccResponse ");
    ...
    Log.e(TAG,"processSendVendorSpecificResultCode ");
    ...
    private String getCurrentDeviceName(BluetoothDevice device) {
    	Log.e(TAG,"getCurrentDeviceName ");
    ...
    public void handleAccessPermissionResult(Intent intent) {
    	Log.e(TAG,"handleAccessPermissionResult ");
