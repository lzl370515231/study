...
public class SapService extends ProfileService {
    ...
    private static final String TAG = "LZLB_SapService";
    ...
    public static void notifyUpdateWakeLock(Handler messageHandler) {
    	Log.e(TAG,"notifyUpdateWakeLock");
    ...
    private void startRfcommSocketListener() {
        if (VERBOSE) Log.e(TAG, "Sap Service startRfcommSocketListener");
    ...
    private final boolean initSocket() {
        if (VERBOSE) Log.e(TAG, "Sap Service initSocket");
    ...
    private final synchronized void closeConnectionSocket() {
    	Log.e(TAG,"closeConnectionSocket");
        if (mConnSocket != null) {
            try {
                mConnSocket.close();
                mConnSocket = null;
    ...
    private final void closeService() {
        if (VERBOSE) Log.e(TAG, "SAP Service closeService in");
    ...
    private final void startSapServerSession() throws IOException {
        if (VERBOSE) Log.e(TAG, "Sap Service startSapServerSession");
    ...
    private class SocketAcceptThread extends Thread {
		...
		private static final String TAG="LZLB_SocketAcceptThread";
        ...
        public void run() {
            ...
            while (!stopped) {
                try {
                    if (VERBOSE) Log.e(TAG, "Accepting socket connection...");
                    ...
                    if (VERBOSE) Log.e(TAG, "Accepted socket connection...");
                    ...
                    if (VERBOSE) Log.e(TAG, "getSimAccessPermission() = " + permission);
    ...
    private synchronized void setState(int state, int result) {
        if (state != mState) {
            if (DEBUG) Log.e(TAG, "Sap state " + mState + " -> " + state + ", result = "
                    + result);
    ...
    private void sendCancelUserConfirmationIntent(BluetoothDevice device) {
		Log.e(TAG,"sendCancelUserConfirmationIntent()");
    ...
    private class SapBroadcastReceiver extends BroadcastReceiver {
        ...
        public void onReceive(Context context, Intent intent) {
            ...
            if (VERBOSE) Log.e(TAG, "onReceive");
    ...
    private static class SapBinder extends IBluetoothSap.Stub
		...
		private static final String TAG ="LZLB_SapBinder";
