...
public class PbapPCEClient  implements PbapHandler.PbapListener {
    private static final String TAG = "LZLB_PbapPCEClient";
    ...
    public int getConnectionState() {
		...
		Log.e(TAG,"getConnectionState currentState:"+currentState);
        switch(currentState) {
          case DISCONNECTED:
    ...
    public void onPhoneBookError() {
        if (DBG) {
            Log.e(TAG, "Error, mLastPull = "  + mLastPull);
        }
        processNextRequest();
    }
    ...
    public synchronized void onPbapClientConnected(boolean status) {
    	Log.e(TAG,"onPbapClientConnected");
    ...
    private class ConnectionHandler extends Handler {
		...
		private static final String TAG ="LZLB_ConnectionHandler";
        ...
        public void handleMessage(Message msg) {
            if (DBG) {
                Log.e(TAG, "Connection Handler Message " + msg.what + " with " + msg.obj);
            }
            switch (msg.what) {
                case EVENT_CONNECT:
        ...
        private void handleConnect(BluetoothDevice device) {
          Log.e(TAG,"HANDLECONNECT" + device);
            if (device == null) {
        ...
        private void handleDisconnect(BluetoothDevice device) {
            Log.e(TAG, "pbap disconnecting from = " + device);
    ...
    private void addPullRequest(PullRequest r) {
        if (DBG) {
            Log.e(TAG, "pull request mClient=" + mClient + " connected= " +
    ...
    private class ContactHandler extends Handler {
        ...
        public void handleMessage(Message msg) {
            if (DBG) {
                Log.e(TAG, "Contact Handler Message " + msg.what + " with " + msg.obj);
            }
            switch (msg.what) {
        ...
        private void removeUncleanAccounts() {
            ...
            Log.e(TAG, "Found " + accounts.length + " unclean accounts");
